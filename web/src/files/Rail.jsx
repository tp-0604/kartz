...
export default function Rail({ narrow }) {
  const { mode, go, user, section, openSection, fileOpen, boards, datasets, openInData } = useApp();
  ...
  return (
    <nav className={'rail' + (narrow ? ' rail--narrow' : '')} aria-label="Where everything is">
      ...
      {sections.map(s => (
        <Item key={s.id || s.name} cover={s.cover} title={s.name}
