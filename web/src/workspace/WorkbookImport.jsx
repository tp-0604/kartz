...
export default function WorkbookImport({ onDone }) {
  ...
  return (
    <div className="stack">
      <div>
        <p className="hint" style={{ marginBottom: 'var(--s3)' }}>In Google Sheets:
          <strong> File → Download → Microsoft Excel (.xlsx)</strong>, then drop the file here.
          Every tab named for a month is read; the roster tab is not, because the app keeps the
          roster itself. The file stays in this browser, and nothing is saved until you press Import.</p>
      ...
      {!plan && !reading && !err && (
        <Empty title="Nothing read yet">
          Drop the workbook above. You will see every board it would create, and the dates it
          would give them, before anything is saved.
        </Empty>
      )}
    </div>
  );
